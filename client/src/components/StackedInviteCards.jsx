...
function getMenuLabel(source, menuUrl) {
  if (!menuUrl) return null;
  switch (source) {
    case "swiggy": return "View on Swiggy";
    case "zomato": return "View on Zomato";
    case "dineout": return "View on Dineout";
...
export default function StackedInviteCards({ invite, calendarUrl, shareUrl }) {
  ...
  const menuLabel = getMenuLabel(invite.source, invite.menuUrl);
  ...
  const cards = useMemo(() => {
          ...
          <div className="flex h-full flex-col justify-between">
            ...
            >
              <img
                ...
                alt={invite.restaurantName || "Event"}
        ...
        content: (
          <div className="flex h-full flex-col items-center justify-center text-center px-6">
            ...
            <motion.h2
              ...
              custom={0.3}
            >
              {invite.restaurantName || "The Venue"}
            </motion.h2>
            <motion.p
            ...
            <motion.div
              ...
              custom={0.54}
            >
              {invite.cuisine && <span>{invite.cuisine}</span>}
              {invite.cuisine && invite.rating && <span>•</span>}
              {invite.rating && (
    ...
    // Insert custom cards after venue
    const customCardElements = (invite.customCards || [])
      .filter((cc) => cc.title || cc.body)
      .map((cc) => ({
        id: cc.id,
        content: (
          <div className="flex h-full flex-col items-center justify-center text-center px-6">
            <motion.div
              className="mb-4 inline-flex items-center justify-center rounded-full bg-gradient-to-br from-violet-100 to-purple-100 p-4"
              variants={lineReveal}
              initial="hidden"
              animate="visible"
              custom={0.1}
            ...
            {cc.body && (
              <motion.p
                className="text-base text-rose-700 leading-relaxed max-w-xs"
                variants={lineReveal}
                initial="hidden"
                animate="visible"
                custom={0.35}
    ...
    // CTA card with dynamic menu label
    const ctaCard = {
      id: "cta",
      content: (
        <div className="flex h-full flex-col items-center justify-center text-center px-6">
      ...
      content: (
        <div className="flex h-full flex-col items-center justify-center text-center px-4">
          ...
          <motion.h2
            ...
            custom={0.3}
          >
            {invite.restaurantName || "Event Location"}
          </motion.h2>
          <motion.div
    ...
    return [...baseCards, ...customCardElements, ctaCard, locationCard];
  }, [invite, calendarUrl, shareUrl, menuLabel]);
  ...
  // Find CTA card index dynamically for confetti trigger
  const ctaIndex = cards.findIndex((c) => c.id === "cta");

  // Trigger confetti when reaching the CTA card
  useEffect(() => {
    if (currentCard === ctaIndex && ctaIndex >= 0 && !hasTriggeredConfetti) {
  ...
  }, [currentCard, ctaIndex, hasTriggeredConfetti]);
  ...
  useEffect(() => {
    ...
    function handleWheel(event) {
      ...
      const maxCard = totalCardsRef.current - 1;
      if (event.deltaY > 0 && currentCard < maxCard) {
